---
title: "Getting Started with chronofeat"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Getting Started with chronofeat}
...
**chronofeat** is an R package for time series feature engineering and forecasting. It provides:

- **Formula-based feature specification**: Define lags, moving averages, rolling statistics, and calendar features using a concise formula syntax
- **Model-agnostic design**: Works with any R model that has a fit/predict interface (lm, glm, xgboost, randomForest, etc.)
- **Recursive multi-step forecasting**: Automatically generates features at each forecast step
- **Panel data support**: Handle multiple time series (groups) in a single workflow
...
1. **Prepare data** with `TimeSeries()` (optional but recommended)
2. **Fit a model** with `fit()` using a feature formula
3. **Generate forecasts** with `forecast()`

### Step 1: Load and Prepare Data

chronofeat includes a sample retail sales dataset with 42 product categories (items) and monthly observations:
...
Create a `TimeSeries` object to bundle the data with frequency information:
...
Use `fit()` with a formula to specify your target variable and features:
...
The model object contains the fitted model and all metadata needed for forecasting:
...
Use `forecast()` to generate multi-step ahead predictions:
...
The forecast is recursive: each predicted value becomes history for generating features at the next step.
...
The formula's left side specifies the target variable. The right side specifies features:
...
chronofeat works with any model that has a standard R interface. Pass the model function directly:

```{r different_models, eval=FALSE}
# Linear regression
...
For more control, provide a custom model specification with `fit` and `predict` functions:
...
See the [Building Custom Models](custom-models.html) article for detailed examples with XGBoost, LightGBM, and other frameworks.

## Panel Data (Multiple Time Series)

chronofeat handles panel data (multiple time series) through the `groups` parameter. Each group gets its own:
...
See the [Cross-Validation](cross-validation.html) article for more details.

## Next Steps

- **[Building Custom Models](custom-models.html)**: Learn how to integrate XGBoost, LightGBM, neural networks, and custom ensembles
- **[Feature Engineering Reference](feature-engineering.html)**: Complete guide to all available features
- **[Data Preprocessing](preprocessing.html)**: Handle missing dates, gaps, and irregular calendars
- **[Cross-Validation](cross-validation.html)**: Proper evaluation workflows for time series
- **[Advanced Workflows](advanced-workflows.html)**: Hyperparameter tuning, model comparison, and production patterns

## Key Functions Reference

| Function | Purpose |
|----------|---------|
| `TimeSeries()` | Create a time series object with preprocessing |
| `fit()` | Fit a forecasting model with feature formula |
| `forecast()` | Generate recursive multi-step forecasts |
| `cv_forecast()` | Time series cross-validation |
| `as_model_spec()` | Convert model function to specification |
